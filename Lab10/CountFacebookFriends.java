...
public static void main(String [] args) throws IOException{
    ...
    Graph<Friend> mreza = new Graph<>(n, friends);
    for (int i=0; i<n; i++){
        int broj = Integer.parseInt(br.readLine());
        for (int j=0; j<broj; j++){
            int num = Integer.parseInt(br.readLine().split("\\s+")[0]);
            mreza.addEdge(i, num);
            mreza.addEdge(num, i);
        }
    }
    ...
    Queue <GraphNode<Friend>> queue = new LinkedQueue<>();
    queue.enqueue(mreza.adjList[start]);
    boolean [] visited = new boolean[n];
    Arrays.fill(visited,false);
    visited[start]=true;
    while (!queue.isEmpty()){
        GraphNode<Friend> top = queue.peek();
        if (top.getIndex()==end){
            System.out.println(top.getInfo().getDistance());
            return;
        }
        queue.dequeue();
        for (int i=0; i<mreza.num_nodes; i++){
            if (mreza.adjacent(i, top.getIndex())==1&&!visited[i]){
                mreza.adjList[i].getInfo().setDistance(top.getInfo().getDistance()+1);
                visited[i]=true;
                queue.enqueue(mreza.adjList[i]);
            }
        }
    }
}
