...
public class ArrayMeanValue {
    ...
    public static int brojDoProsek(Array<Integer> arr) {
    	...
    	for (int i=0; i<arr.getSize(); i++)
        	avg+=(int)arr.get(i);
    	avg/=arr.getSize();
    	float min=Math.abs(arr.get(0)-avg);
    	int najblisku=arr.get(0);
    	for (int i=1; i<arr.getSize(); i++) {
    		if (Math.abs(arr.get(i)-avg)<min){
    			min=Math.abs(arr.get(i)-avg);
    			najblisku=arr.get(i);
    		}
    		if (Math.abs(arr.get(i)-avg)==min&&arr.get(i)<najblisku) {
    			min=Math.abs(arr.get(i)-avg);
    			najblisku=arr.get(i);
    		}
    	}
