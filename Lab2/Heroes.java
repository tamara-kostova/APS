...
Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
    	...
    	int max=curr.element.getPower()*curr.element.getNumAttacks();
    	for (int i=1; i<6; i++) {
    		curr=curr.succ;
    		if (curr.element.getPower()*curr.element.getNumAttacks()>max) {
    			max=curr.element.getPower()*curr.element.getNumAttacks();
    			maxcard=curr;
    		}
    	}
    	curr=firstFriendCards.getFirst();
